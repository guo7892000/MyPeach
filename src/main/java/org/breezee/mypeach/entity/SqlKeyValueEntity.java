...
public class SqlKeyValueEntity {
    ...
    public static SqlKeyValueEntity build(String sKeyString, Map<String,Object> dicQuery, MyPeachProperties prop, boolean isPreGetCondition){
        ...
        entity.setKeyMoreInfo(KeyMoreInfo.build(sParamNameMore,inValue,entity));//设置更多信息对象
        ...
        if (ToolHelper.IsNotNull(sInString) && !entity.isHasSingleQuotes()) {
            entity.getKeyMoreInfo().setInString(sInString.replace("'", "").trim());//去掉IN清单字符中的引号
        }
